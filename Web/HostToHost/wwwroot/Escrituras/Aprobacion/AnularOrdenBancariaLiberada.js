...
function fcMostrarOrdenBancariaPorAprobar(ordenBancaria) {
    try {
        ...
        $("#btnAnular").on("click", function () {
            fcAnularOrdenBancariaLiberada(ordenBancaria.idSociedad, ordenBancaria.idSap, ordenBancaria.anio, ordenBancaria.momentoOrden, ordenBancaria.idTipoOrden, ordenBancaria.idEstadoOrden, ordenBancaria.tipoOrdenCorto);
        });
...
function fcMostrarOrdenBancariaTransferenciaBcr(ordenBancaria, listaOrdenesBancariasDetalle) {
    try {
        ...
        for (var i = 0; i < listaOrdenesBancariasDetalle.length; i++)
        {
            ...
            tabla += "<td><div class=\"d-flex\"><i class=\"material-icons cursor\" onclick=\"fcAnularOrdenBancariaDetalleLiberada('" + ordenBancaria.idTipoOrden + "', '" + string64OrdenBancaria + "', '" + string64OrdenBancariaDetalle + "');\" title=\"Ver Pago\" >search</i>";
            tabla += "</div></td>";
            tabla += "</tr>";
        }
...
function fcAnularOrdenBancariaDetalleLiberada(idTipoOrden, string64OrdenBancaria, string64OrdenBancariaDetalle) {
    try {
        ...
        var url = felix.format(constante.APROBACION_ANULAR_ORDEN_BANCARIA_DETALLE_LIBERADA, idTipoOrden, string64OrdenBancaria, string64OrdenBancariaDetalle);
...
function fcAnularOrdenBancariaLiberada(idSociedad, idSap, anio, momentoOrden, idTipoOrden, idEstadoOrden, tipoOrden) {
    try {
        ...
        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_PUT, constante.APROBACION_ANULAR_ORDEN_BANCARIA_LIBERADA_ASYNC, parametros, function (res) {
            if (res !== "")
            {
                if (res.codigo === constante.CODIGO_OK)
                {
