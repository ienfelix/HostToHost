...
function fcCargaInicial() {
    try {
        fcListarOrdenesBancariasAnuladas(constante.PAGINA_CLICADA, constante.PRIMERA_PAGINA, constante.CANTIDAD_FILAS, constante.VACIO, constante.VACIO, constante.VACIO);
        fcListarFiltrosOrdenesBancariasAnuladas();

        $("#btnBuscar").on("click", function () {
            fcBuscarOrdenesBancariasAnuladas();
        });
...
function fcListarOrdenesBancariasAnuladas(accion, pagina, filas, idSociedad, idBanco, idTipoOrden) {
    try {
        ...
        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.APROBACION_LISTAR_ORDENES_BANCARIAS_ANULADAS_ASYNC, parametros, function (res) {
            if (res !== "")
            {
...
function fcConstruirBandeja(listaOrdenesBancarias) {
    try {
        ...
        for (var i = 0; i < listaOrdenesBancarias.length; i++)
        {
            ...
            tabla += "<td><div class=\"d-flex\"><i class=\"material-icons cursor\" onclick=\"fcVerOrdenBancariaAnulada('" + string64OrdenBancaria + "');\" title=\"Ver Orden Bancaria Anulada\" >search</i></div></td>";
            tabla += "</tr>";
        }
...
function fcBuscarOrdenesBancariasAnuladas() {
    try {
        ...
        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.APROBACION_BUSCAR_ORDENES_BANCARIAS_ANULADAS_ASYNC, parametros, function (res) {
            if (res !== "")
            {
                if (res.codigo === constante.CODIGO_OK)
                {
...
function fcConstruirPaginado(accion, pagina, totalRegistros) {
    try {
        ...
        paginado += "<li class\"page-item\"><a class=\"page-link\" href=\"javascript:void(0);\" onclick=\"fcListarOrdenesBancariasAnuladasPaginado('" + constante.PAGINA_ANTERIOR + "', " + paginaAnterior + ", " + constante.CANTIDAD_FILAS + ", '', '', '');\"><span aria-hidden=\"true\">&larr;</span></a></li>";
        
        for (var i = contador; i <= totalPaginas; i++)
        {
            paginado += "<li class=\"page-item correlativo\"><a class=\"page-link\" href=\"javascript:void(0);\" onclick=\"fcListarOrdenesBancariasAnuladasPaginado('" + constante.PAGINA_CLICADA + "', " + i + ", " + constante.CANTIDAD_FILAS + ", '', '', '');\">" + i + "</a></li>";
        }

        paginado += "<li class\"page-item\"><a class=\"page-link\" href=\"javascript:void(0);\" onclick=\"fcListarOrdenesBancariasAnuladasPaginado('" + constante.PAGINA_SIGUIENTE + "', " + paginaSiguiente + ", " + constante.CANTIDAD_FILAS + ", '', '', '');\"><span aria-hidden=\"true\">&rarr;</span></a></li>";
...
function fcListarOrdenesBancariasAnuladasPaginado(accion, pagina, filas, idSociedad, idBanco, idTipoOrden) {
    try {
        ...
        fcListarOrdenesBancariasAnuladas(accion, pagina, filas, idSociedad, idBanco, idTipoOrden);
    } catch (e) {
        felix.fcAlert(constante.TITULO_ERROR, e.message);
    }
}

function fcListarFiltrosOrdenesBancariasAnuladas() {
    try {
        var parametros;

        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.APROBACION_LISTAR_FILTROS_ORDENES_BANCARIAS_ANULADAS_ASYNC, parametros, function (res) {
            if (res !== "")
            {
                if (res.codigo === constante.CODIGO_OK)
                {
    ...
    try {
        ...
        for (var i = 0; i < listaFiltros.length; i++)
        {
            ...
            for (var j = 0; j < categoria.listaOpciones.length; j++)
            {
                ...
                child += "<label class=\"form-check-label\" for=\"" + id + "\" onclick=\"fcListarOrdenesBancariasAnuladasFiltrado('" + categoria.categoria + "', '" + constante.PAGINA_FILTRADA + "', " + constante.PRIMERA_PAGINA + ", " + constante.CANTIDAD_FILAS + ", '" + idSociedad + "', '" + idBanco + "', '" + idTipoOrden + "');\">" + descripcion + "</label>";
                child += "</div>";
                child += "</li>";
                parent += child;
            }
...
function fcListarOrdenesBancariasAnuladasFiltrado(categoria, accion, pagina, filas, idSociedad, idBanco, idTipoOrden) {
    try {
...
function fcVerOrdenBancariaAnulada(string64OrdenBancaria) {
    try {
        ...
        var url = felix.format(constante.APROBACION_VER_ORDEN_BANCARIA_ANULADA, string64OrdenBancaria);
...
function fcExportarHaciaExcel() {
    try {
        felix.fcExportarHaciaExcel(constante.TABLA_ORDEN_BANCARIA, constante.ORDENES_BANCARIAS_ANULADAS, constante.ORDENES_BANCARIAS_ANULADAS);
    } catch (e) {
        felix.fcAlert(constante.TITULO_ERROR, e.message);
    }
}