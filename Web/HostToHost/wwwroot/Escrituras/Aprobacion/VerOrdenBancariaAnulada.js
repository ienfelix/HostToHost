    ...
    try {
        ...
        else
        {
            ...
            else
            {
                ...
                fcObtenerVistaParcialOrdenBancariaAnulada(ordenBancaria);
            }
        }
        ...
        if (esCorrecto === false)
        {
            felix.fcAlertRedirect(constante.TITULO_MENSAJE, constante.MENSAJE_PARAMETROS_NO_PRESENTES, function () {
                window.location.href = constante.HREF_APROBACION_LISTAR_ORDENES_BANCARIAS_ANULADAS;
            });
        }
    } catch (e) {
...
function fcObtenerVistaParcialOrdenBancariaAnulada(ordenBancaria) {
    try {
        ...
        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.APROBACION_OBTENER_VISTA_PARCIAL_ORDEN_BANCARIA_ANULADA_ASYNC, parametros, function (res) {
            if (res !== "")
            {
                if (res.codigo === constante.CODIGO_OK)
                {
                    $("#dvOrdenBancaria").replaceWith(res.vistaParcialOrdenBancariaAnulada);
                    $("#dvAccionesOrdenBancaria").replaceWith(res.vistaParcialAccionesOrdenBancaria);
                    fcMostrarOrdenBancariaAnulada(ordenBancaria);
...
function fcMostrarOrdenBancariaAnulada(ordenBancaria) {
    try {
        if (typeof ordenBancaria === "undefined" || ordenBancaria.length === 0)
        {
            felix.fcAlert(constante.TITULO_MENSAJE, constante.MENSAJE_PARAMETROS_NO_PRESENTES);
        ...
        $("#btnDeshacer").hide();
        $("#btnAnular").hide();
        $("#btnRegresar").attr("href", constante.HREF_APROBACION_LISTAR_ORDENES_BANCARIAS_ANULADAS);
        ...
        fcListarOrdenesBancariasDetalleAnuladas(ordenBancaria);
    } catch (e) {
        felix.fcAlert(constante.TITULO_ERROR, e.message);
    }
}

function fcListarOrdenesBancariasDetalleAnuladas(ordenBancaria)
{
    try {
        if (typeof ordenBancaria === "undefined" || ordenBancaria.length === 0)
        {
        ...
        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.APROBACION_LISTAR_ORDENES_BANCARIAS_DETALLE_ANULADAS_ASYNC, parametros, function (res) {
            if (res !== "")
            {
                if (res.codigo === constante.CODIGO_OK)
                {
...
function fcMostrarOrdenBancariaTransferencia(ordenBancaria, ordenBancariaDetalle) {
    try {
        ...
        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.APROBACION_OBTENER_VISTA_PARCIAL_ORDEN_BANCARIA_DETALLE_ANULADA_ASYNC, parametros, function (res) {
            if (res !== "")
            {
                if (res.codigo === constante.CODIGO_OK)
                {
                    $("#dvOrdenBancariaDetalle").replaceWith(res.vistaParcialOrdenBancariaDetalleAnulada);
...
function fcMostrarOrdenBancariaTransferenciaBcr(ordenBancaria, listaOrdenesBancariasDetalle) {
    try {
        ...
        for (var i = 0; i < listaOrdenesBancariasDetalle.length; i++)
        {
            ...
            tabla += "<td><div class=\"d-flex\"><i class=\"material-icons cursor\" onclick=\"fcVerOrdenBancariaDetalleAnulada('" + ordenBancaria.idTipoOrden + "', '" + string64OrdenBancaria + "', '" + string64OrdenBancariaDetalle + "');\" title=\"Ver Pago\" >search</i>";
            tabla += "</div></td>";
            tabla += "</tr>";
        }
...
function fcVerOrdenBancariaDetalleAnulada(idTipoOrden, string64OrdenBancaria, string64OrdenBancariaDetalle) {
    try {
        ...
        var url = felix.format(constante.APROBACION_VER_ORDEN_BANCARIA_DETALLE_ANULADA, idTipoOrden, string64OrdenBancaria, string64OrdenBancariaDetalle);
