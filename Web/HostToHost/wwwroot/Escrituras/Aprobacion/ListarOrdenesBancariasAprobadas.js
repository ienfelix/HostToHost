...
function fcCargaInicial() {
    try {
        fcListarOrdenesBancariasAprobadas(constante.PAGINA_CLICADA, constante.PRIMERA_PAGINA, constante.CANTIDAD_FILAS, constante.VACIO, constante.VACIO, constante.VACIO);
        fcListarFiltrosOrdenesBancariasAprobadas();

        $("#btnBuscar").on("click", function () {
            fcBuscarOrdenesBancariasAprobadas();
        });
...
function fcListarOrdenesBancariasAprobadas(accion, pagina, filas, idSociedad, idBanco, idTipoOrden) {
    try {
        ...
        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.APROBACION_LISTAR_ORDENES_BANCARIAS_APROBADAS_ASYNC, parametros, function (res) {
            if (res !== "")
            {
...
function fcConstruirBandeja(listaOrdenesBancarias) {
    try {
        ...
        for (var i = 0; i < listaOrdenesBancarias.length; i++)
        {
            ...
            tabla += "<td><div class=\"d-flex\"><i class=\"material-icons cursor\" onclick=\"fcVerOrdenBancariaAprobada('" + string64OrdenBancaria + "');\" title=\"Ver Orden Bancaria Aprobada\" >search</i></div></td>";
            tabla += "</tr>";
        }
...
function fcBuscarOrdenesBancariasAprobadas() {
    try {
        ...
        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.APROBACION_BUSCAR_ORDENES_BANCARIAS_APROBADAS_ASYNC, parametros, function (res) {
            if (res !== "")
            {
                if (res.codigo === constante.CODIGO_OK)
                {
...
function fcListarOrdenesBancariasAprobadasPaginado(accion, pagina, filas, idSociedad, idBanco, idTipoOrden) {
    try {
        ...
        fcListarOrdenesBancariasAprobadas(accion, pagina, filas, idSociedad, idBanco, idTipoOrden);
    } catch (e) {
        felix.fcAlert(constante.TITULO_ERROR, e.message);
    }
}

function fcListarFiltrosOrdenesBancariasAprobadas() {
    try {
        var parametros;

        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.APROBACION_LISTAR_FILTROS_ORDENES_BANCARIAS_APROBADAS_ASYNC, parametros, function (res) {
            if (res !== "")
            {
                if (res.codigo === constante.CODIGO_OK)
                {
    ...
    try {
        ...
        for (var i = 0; i < listaFiltros.length; i++)
        {
            ...
            for (var j = 0; j < categoria.listaOpciones.length; j++)
            {
                ...
                child += "<label class=\"form-check-label\" for=\"" + id + "\" onclick=\"fcListarOrdenesBancariasAprobadasFiltrado('" + categoria.categoria + "', '" + constante.PAGINA_FILTRADA + "', " + constante.PRIMERA_PAGINA + ", " + constante.CANTIDAD_FILAS + ", '" + idSociedad + "', '" + idBanco + "', '" + idTipoOrden + "');\">" + descripcion + "</label>";
                child += "</div>";
                child += "</li>";
                parent += child;
            }
...
function fcListarOrdenesBancariasAprobadasFiltrado(categoria, accion, pagina, filas, idSociedad, idBanco, idTipoOrden) {
    try {
...
function fcVerOrdenBancariaAprobada(string64OrdenBancaria) {
    try {
        ...
        var url = felix.format(constante.APROBACION_VER_ORDEN_BANCARIA_APROBADA, string64OrdenBancaria);
...
function fcExportarHaciaExcel() {
    try {
        felix.fcExportarHaciaExcel(constante.TABLA_ORDEN_BANCARIA, constante.ORDENES_BANCARIAS_APROBADAS, constante.ORDENES_BANCARIAS_APROBADAS);
    } catch (e) {
        felix.fcAlert(constante.TITULO_ERROR, e.message);
    }
}