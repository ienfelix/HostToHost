...
function fcCargaInicial() {
    try {
        fcListarOrdenesBancariasLiberadas(constante.PAGINA_CLICADA, constante.PRIMERA_PAGINA, constante.CANTIDAD_FILAS, constante.VACIO, constante.VACIO, constante.VACIO);
        fcListarFiltrosOrdenesBancariasLiberadas();

        $("#btnBuscar").on("click", function () {
            fcBuscarOrdenesBancariasLiberadas();
        });
...
function fcListarOrdenesBancariasLiberadas(accion, pagina, filas, idSociedad, idBanco, idTipoOrden) {
    try {
        ...
        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.TESORERIA_LISTAR_ORDENES_BANCARIAS_LIBERADAS_ASYNC, parametros, function (res) {
            if (res !== "")
            {
...
function fcConstruirBandeja(listaOrdenesBancarias) {
    try {
        ...
        for (var i = 0; i < listaOrdenesBancarias.length; i++)
        {
            ...
            tabla += "<td>" + listaOrdenesBancarias[i].usuarioCreacion + "</td>";
            tabla += "<td>" + listaOrdenesBancarias[i].idSap + "</td>";
            tabla += "<td align=\"right\">" + listaOrdenesBancarias[i].estadoOrden + "</td>";
            ...
            tabla += "<td><div class=\"d-flex\"><i class=\"material-icons cursor\" onclick=\"fcVerOrdenBancariaLiberada('" + string64OrdenBancaria + "');\" title=\"Ver Orden Bancaria Liberada\" >search</i>";
            tabla += "<i class=\"material-icons cursor\" onclick=\"fcDeshacerOrdenBancariaLiberada('" + string64OrdenBancaria + "');\" title=\"Anular Orden Bancaria Liberada\" >delete</i></div></td>";
            tabla += "</tr>";
        }
...
function fcBuscarOrdenesBancariasLiberadas() {
    try {
        ...
        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.TESORERIA_BUSCAR_ORDENES_BANCARIAS_LIBERADAS_ASYNC, parametros, function (res) {
            if (res !== "")
            {
                if (res.codigo === constante.CODIGO_OK)
                {
...
function fcConstruirPaginado(accion, pagina, totalRegistros) {
    try {
        ...
        paginado += "<li class\"page-item\"><a class=\"page-link\" href=\"javascript:void(0);\" onclick=\"fcListarOrdenesBancariasLiberadasPaginado('" + constante.PAGINA_ANTERIOR + "', " + paginaAnterior + ", " + constante.CANTIDAD_FILAS + ", '', '', '');\"><span aria-hidden=\"true\">&larr;</span></a></li>";
        
        for (var i = contador; i <= totalPaginas; i++)
        {
            paginado += "<li class=\"page-item correlativo\"><a class=\"page-link\" href=\"javascript:void(0);\" onclick=\"fcListarOrdenesBancariasLiberadasPaginado('" + constante.PAGINA_CLICADA + "', " + i + ", " + constante.CANTIDAD_FILAS + ", '', '', '');\">" + i + "</a></li>";
        }

        paginado += "<li class\"page-item\"><a class=\"page-link\" href=\"javascript:void(0);\" onclick=\"fcListarOrdenesBancariasLiberadasPaginado('" + constante.PAGINA_SIGUIENTE + "', " + paginaSiguiente + ", " + constante.CANTIDAD_FILAS + ", '', '', '');\"><span aria-hidden=\"true\">&rarr;</span></a></li>";
...
function fcListarOrdenesBancariasLiberadasPaginado(accion, pagina, filas, idSociedad, idBanco, idTipoOrden) {
    try {
        ...
        fcListarOrdenesBancariasLiberadas(accion, pagina, filas, idSociedad, idBanco, idTipoOrden);
    } catch (e) {
        felix.fcAlert(constante.TITULO_ERROR, e.message);
    }
}

function fcListarFiltrosOrdenesBancariasLiberadas() {
    try {
        var parametros;

        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.TESORERIA_LISTAR_FILTROS_ORDENES_BANCARIAS_LIBERADAS_ASYNC, parametros, function (res) {
            if (res !== "")
            {
                if (res.codigo === constante.CODIGO_OK)
                {
    ...
    try {
        ...
        for (var i = 0; i < listaFiltros.length; i++)
        {
            ...
            for (var j = 0; j < categoria.listaOpciones.length; j++)
            {
                ...
                child += "<label class=\"form-check-label\" for=\"" + id + "\" onclick=\"fcListarOrdenesBancariasLiberadasFiltrado('" + categoria.categoria + "', '" + constante.PAGINA_FILTRADA + "', " + constante.PRIMERA_PAGINA + ", " + constante.CANTIDAD_FILAS + ", '" + idSociedad + "', '" + idBanco + "', '" + idTipoOrden + "');\">" + descripcion + "</label>";
                child += "</div>";
                child += "</li>";
                parent += child;
            }
...
function fcListarOrdenesBancariasLiberadasFiltrado(categoria, accion, pagina, filas, idSociedad, idBanco, idTipoOrden) {
    try {
...
function fcVerOrdenBancariaLiberada(string64OrdenBancaria) {
    try {
        ...
        var url = felix.format(constante.TESORERIA_VER_ORDEN_BANCARIA_LIBERADA, string64OrdenBancaria);
...
function fcDeshacerOrdenBancariaLiberada(string64OrdenBancaria) {
    try {
        if (typeof string64OrdenBancaria === "undefined" || string64OrdenBancaria == "")
        {
            felix.fcAlert(constante.TITULO_MENSAJE, constante.MENSAJE_PARAMETROS_NO_PRESENTES);
        ...
        var url = felix.format(constante.TESORERIA_DESHACER_ORDEN_BANCARIA_LIBERADA, string64OrdenBancaria);
        window.location.href = url;
    } catch (e) {
        felix.fcAlert(constante.TITULO_ERROR, e.message);
    }
...
function fcExportarHaciaExcel() {
    try {
        felix.fcExportarHaciaExcel(constante.TABLA_ORDEN_BANCARIA, constante.ORDENES_BANCARIAS_LIBERADAS, constante.ORDENES_BANCARIAS_LIBERADAS);
    } catch (e) {
        felix.fcAlert(constante.TITULO_ERROR, e.message);
    }
}