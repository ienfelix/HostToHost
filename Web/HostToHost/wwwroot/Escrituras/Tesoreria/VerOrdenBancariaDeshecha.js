    ...
    try {
        ...
        else
        {
            ...
            else
            {
                ...
                fcObtenerVistaParcialOrdenBancariaDeshecha(ordenBancaria);
            }
        }
        ...
        if (esCorrecto === false)
        {
            felix.fcAlertRedirect(constante.TITULO_MENSAJE, constante.MENSAJE_PARAMETROS_NO_PRESENTES, function () {
                window.location.href = constante.HREF_TESORERIA_LISTAR_ORDENES_BANCARIAS_DESHECHAS;
            });
        }
    } catch (e) {
...
function fcObtenerVistaParcialOrdenBancariaDeshecha(ordenBancaria) {
    try {
        ...
        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.TESORERIA_OBTENER_VISTA_PARCIAL_ORDEN_BANCARIA_DESHECHA_ASYNC, parametros, function (res) {
            if (res !== "")
            {
                if (res.codigo === constante.CODIGO_OK)
                {
                    $("#dvOrdenBancaria").replaceWith(res.vistaParcialOrdenBancariaDeshecha);
                    $("#dvAccionesOrdenBancaria").replaceWith(res.vistaParcialAccionesOrdenBancaria);
                    fcMostrarOrdenBancariaDeshecha(ordenBancaria);
...
function fcMostrarOrdenBancariaDeshecha(ordenBancaria) {
    try {
        if (typeof ordenBancaria === "undefined" || ordenBancaria.length === 0)
        {
            felix.fcAlert(constante.TITULO_MENSAJE, constante.MENSAJE_PARAMETROS_NO_PRESENTES);
        ...
        $("#btnDeshacer").hide();
        $("#btnAnular").hide();
        $("#btnRegresar").attr("href", constante.HREF_TESORERIA_LISTAR_ORDENES_BANCARIAS_DESHECHAS);
        ...
        fcListarOrdenesBancariasDetalleDeshechas(ordenBancaria);
    } catch (e) {
        felix.fcAlert(constante.TITULO_ERROR, e.message);
    }
}

function fcListarOrdenesBancariasDetalleDeshechas(ordenBancaria)
{
    try {
        if (typeof ordenBancaria === "undefined" || ordenBancaria.length === 0)
        {
        ...
        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.TESORERIA_LISTAR_ORDENES_BANCARIAS_DETALLE_DESHECHAS_ASYNC, parametros, function (res) {
            if (res !== "")
            {
                if (res.codigo === constante.CODIGO_OK)
                {
...
function fcMostrarOrdenBancariaTransferencia(ordenBancaria, ordenBancariaDetalle) {
    try {
        ...
        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.TESORERIA_OBTENER_VISTA_PARCIAL_ORDEN_BANCARIA_DETALLE_DESHECHA_ASYNC, parametros, function (res) {
            if (res !== "")
            {
                if (res.codigo === constante.CODIGO_OK)
                {
                    $("#dvOrdenBancariaDetalle").replaceWith(res.vistaParcialOrdenBancariaDetalleDeshecha);
...
function fcMostrarOrdenBancariaTransferenciaBcr(ordenBancaria, listaOrdenesBancariasDetalle) {
    try {
        ...
        for (var i = 0; i < listaOrdenesBancariasDetalle.length; i++)
        {
            ...
            tabla += "<td><div class=\"d-flex\"><i class=\"material-icons cursor\" onclick=\"fcVerOrdenBancariaDetalleDeshecha('" + ordenBancaria.idTipoOrden + "', '" + string64OrdenBancaria + "', '" + string64OrdenBancariaDetalle + "');\" title=\"Ver Factura\" >search</i>";
            tabla += "</div></td>";
            tabla += "</tr>";
        }
...
function fcVerOrdenBancariaDetalleDeshecha(idTipoOrden, string64OrdenBancaria, string64OrdenBancariaDetalle) {
    try {
        ...
        var url = felix.format(constante.TESORERIA_VER_ORDEN_BANCARIA_DETALLE_DESHECHA, idTipoOrden, string64OrdenBancaria, string64OrdenBancariaDetalle);
