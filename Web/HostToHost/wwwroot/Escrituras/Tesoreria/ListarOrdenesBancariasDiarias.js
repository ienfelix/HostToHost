...
function fcCargaInicial() {
    try {
        fcListarOrdenesBancariasDiarias(constante.PAGINA_CLICADA, constante.PRIMERA_PAGINA, constante.CANTIDAD_FILAS, constante.VACIO, constante.VACIO, constante.VACIO, constante.VACIO);
        fcListarFiltrosOrdenesBancariasDiarias();

        $("#btnBuscar").on("click", function () {
            fcBuscarOrdenesBancariasDiarias();
        });
        ...
        $("#btnAprobar").hide();
        var dvFechaInicio = $("#lblFechaInicio").closest("div");
        var dvFechaFin = $("#lblFechaFin").closest("div");
        $(dvFechaInicio).hide();
        $(dvFechaFin).hide();
...
function fcListarOrdenesBancariasDiarias(accion, pagina, filas, idSociedad, idBanco, idTipoOrden, idEstadoOrden) {
    try {
        ...
        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.TESORERIA_LISTAR_ORDENES_BANCARIAS_DIARIAS_ASYNC, parametros, function (res) {
            if (res !== "")
            {
...
function fcConstruirBandeja(listaOrdenesBancarias) {
    try {
        ...
        for (var i = 0; i < listaOrdenesBancarias.length; i++)
        {
            ...
            tabla += "<td>" + listaOrdenesBancarias[i].aprobador + "</td>";
            ...
            tabla += "<td><div class=\"d-flex\"><i class=\"material-icons cursor\" onclick=\"fcVerOrdenBancariaDiaria('" + string64OrdenBancaria + "');\" title=\"Ver Orden Bancaria Diaria\" >search</i></div></td>";
            tabla += "</tr>";
        }
...
function fcBuscarOrdenesBancariasDiarias() {
    try {
        var idSap = $("#txtIdSap").val().trim();
        var mensaje = "";
        ...
        var parametros = felix.format("{0}{1}{2}{3}", constante.QUESTION, constante.ID_SAP, constante.EQUAL, idSap);
        
        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.TESORERIA_BUSCAR_ORDENES_BANCARIAS_DIARIAS_ASYNC, parametros, function (res) {
            if (res !== "")
            {
                if (res.codigo === constante.CODIGO_OK)
                {
...
function fcConstruirPaginado(accion, pagina, totalRegistros) {
    try {
        ...
        paginado += "<li class\"page-item\"><a class=\"page-link\" href=\"javascript:void(0);\" onclick=\"fcListarOrdenesBancariasDiariasPaginado('" + constante.PAGINA_ANTERIOR + "', " + paginaAnterior + ", " + constante.CANTIDAD_FILAS + ", '', '', '', '');\"><span aria-hidden=\"true\">&larr;</span></a></li>";
        
        for (var i = contador; i <= totalPaginas; i++)
        {
            paginado += "<li class=\"page-item correlativo\"><a class=\"page-link\" href=\"javascript:void(0);\" onclick=\"fcListarOrdenesBancariasDiariasPaginado('" + constante.PAGINA_CLICADA + "', " + i + ", " + constante.CANTIDAD_FILAS + ", '', '', '', '');\">" + i + "</a></li>";
        }

        paginado += "<li class\"page-item\"><a class=\"page-link\" href=\"javascript:void(0);\" onclick=\"fcListarOrdenesBancariasDiariasPaginado('" + constante.PAGINA_SIGUIENTE + "', " + paginaSiguiente + ", " + constante.CANTIDAD_FILAS + ", '', '', '', '');\"><span aria-hidden=\"true\">&rarr;</span></a></li>";
...
function fcListarOrdenesBancariasDiariasPaginado(accion, pagina, filas, idSociedad, idBanco, idTipoOrden, idEstadoOrden) {
    try {
        ...
        fcListarOrdenesBancariasDiarias(accion, pagina, filas, idSociedad, idBanco, idTipoOrden, idEstadoOrden);
    } catch (e) {
        felix.fcAlert(constante.TITULO_ERROR, e.message);
    }
}

function fcListarFiltrosOrdenesBancariasDiarias() {
    try {
        var parametros;

        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.TESORERIA_LISTAR_FILTROS_ORDENES_BANCARIAS_DIARIAS_ASYNC, parametros, function (res) {
            if (res !== "")
            {
                if (res.codigo === constante.CODIGO_OK)
                {
    ...
    try {
        ...
        for (var i = 0; i < listaFiltros.length; i++)
        {
            ...
            for (var j = 0; j < categoria.listaOpciones.length; j++)
            {
                ...
                child += "<label class=\"form-check-label\" for=\"" + id + "\" onclick=\"fcListarOrdenesBancariasDiariasFiltrado('" + categoria.categoria + "', '" + constante.PAGINA_FILTRADA + "', " + constante.PRIMERA_PAGINA + ", " + constante.CANTIDAD_FILAS + ", '" + idSociedad + "', '" + idBanco + "', '" + idTipoOrden + "', '" + idEstadoOrden + "');\">" + descripcion + "</label>";
                child += "</div>";
                child += "</li>";
                parent += child;
            }
...
function fcListarOrdenesBancariasDiariasFiltrado(categoria, accion, pagina, filas, idSociedad, idBanco, idTipoOrden, idEstadoOrden) {
    try {
...
function fcVerOrdenBancariaDiaria(string64OrdenBancaria) {
    try {
...
function fcExportarHaciaExcel() {
    try {
        felix.fcExportarHaciaExcel(constante.TABLA_ORDEN_BANCARIA, constante.ORDENES_BANCARIAS_DIARIAS, constante.ORDENES_BANCARIAS_DIARIAS);
    } catch (e) {
        felix.fcAlert(constante.TITULO_ERROR, e.message);
    }
}