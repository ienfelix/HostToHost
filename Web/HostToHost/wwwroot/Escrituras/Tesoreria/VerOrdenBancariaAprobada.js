    ...
    try {
        ...
        if (esCorrecto === false)
        {
            felix.fcAlertRedirect(constante.TITULO_MENSAJE, constante.MENSAJE_PARAMETROS_NO_PRESENTES, function () {
                window.location.href = constante.HREF_TESORERIA_LISTAR_ORDENES_BANCARIAS_APROBADAS;
            });
        }
    } catch (e) {
...
function fcObtenerVistaParcialOrdenBancariaAprobada(ordenBancaria) {
    try {
        ...
        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.TESORERIA_OBTENER_VISTA_PARCIAL_ORDEN_BANCARIA_APROBADA_ASYNC, parametros, function (res) {
            if (res !== "")
            {
...
function fcMostrarOrdenBancariaAprobada(ordenBancaria) {
    try {
        ...
        $("#btnDeshacer").hide();
        $("#btnAnular").hide();
        $("#btnRegresar").attr("href", constante.HREF_TESORERIA_LISTAR_ORDENES_BANCARIAS_APROBADAS);
...
function fcListarOrdenesBancariasDetalleAprobadas(ordenBancaria)
{
    try {
        ...
        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.TESORERIA_LISTAR_ORDENES_BANCARIAS_DETALLE_APROBADAS_ASYNC, parametros, function (res) {
            if (res !== "")
            {
                if (res.codigo === constante.CODIGO_OK)
                {
...
function fcMostrarOrdenBancariaTransferencia(ordenBancaria, ordenBancariaDetalle) {
    try {
        ...
        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.TESORERIA_OBTENER_VISTA_PARCIAL_ORDEN_BANCARIA_DETALLE_APROBADA_ASYNC, parametros, function (res) {
            if (res !== "")
            {
                if (res.codigo === constante.CODIGO_OK)
                {
...
function fcVerOrdenBancariaDetalleAprobada(idTipoOrden, string64OrdenBancaria, string64OrdenBancariaDetalle) {
    try {
        ...
        var url = felix.format(constante.TESORERIA_VER_ORDEN_BANCARIA_DETALLE_APROBADA, idTipoOrden, string64OrdenBancaria, string64OrdenBancariaDetalle);
