...
function fcListarUsuarios(accion, pagina, filas) {
    try {
        ...
        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.ROL_LISTAR_USUARIOS_ASYNC, parametros, function (res) {
            if (res !== "")
            {
...
function fcConstruirBandeja(listaUsuarios) {
    try {
        ...
        for (var i = 0; i < listaUsuarios.length; i++)
        {
            ...
            tabla += "<i class=\"material-icons cursor\" onclick=\"fcAsignarRol('" + string64Usuario + "');\" title=\"Asignar Rol\" >assignment</i>";
            tabla += "<i class=\"material-icons cursor\" onclick=\"fcDesasignarRol('" + string64Usuario + "');\" title=\"Desasignar Rol\" >assignment_late</i>";
            tabla += "</div></td></tr>";
        }
...
function fcBuscarUsuarios() {
    try {
        ...
        felix.fcHttpClient(constante.ASYNC_TRUE, constante.HTTP_GET, constante.ROL_BUSCAR_USUARIOS_ASYNC, parametros, function (res) {
            if (res !== "")
            {
                if (res.codigo === constante.CODIGO_OK)
                {
...
function fcAsignarRol(string64Usuario) {
    try {
        ...
        var url = felix.format(constante.ROL_ASIGNAR_ROL, string64Usuario);
...
function fcDesasignarRol(string64Usuario) {
    try {
        if (typeof string64Usuario === "undefined" || string64Usuario == "")
        {
            felix.fcAlert(constante.TITULO_MENSAJE, constante.MENSAJE_PARAMETROS_NO_PRESENTES);
        ...
        var url = felix.format(constante.ROL_DESASIGNAR_ROL, string64Usuario);
        window.location.href = url;
    } catch (e) {
        felix.fcAlert(constante.TITULO_ERROR, e.message);
    }
